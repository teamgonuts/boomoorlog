...
Stand up the database and make it the single source for trees, genera, and stats.
**Deliverables**
- [ ] DB schema: `genera`/stats, `trees`, postcodes (+ boundary geometry only if needed).
- [ ] Migration + seed scripts that load from existing CSV/GeoJSON into Postgres.
- [ ] A documented query layer / typed access (the `Unit`/`Army` contract starts here).
- [ ] CSVs demoted to raw pipeline inputs — never read by the app.
- [ ] Verify: query "trees in ZIP X" and "stats for genus Y" against the live DB.
